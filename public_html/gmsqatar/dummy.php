...
<!-- jQuery (Already present if you're using it) -->
...
<select name="partName[]" id="partName1" class="form-control-sm partNameDropdown select2" onchange="setPartsData(1)">
...
<div class="form-inline justify-content-between">
<div>
<input type="button" class="btn btn-primary" id="addPackage" value="Add Packages" style="margin-right: 10px;">    
...
$("#addPart").click(function () {
    partIndex++;
    let html_code = `
<tr id="partRow${partIndex}">
<td>
  <input type="hidden" class="partIndex" name="partIndex[]" value="${partIndex}" readonly/>
  <input type="hidden" class="partId" name="partId[]" id="partId${partIndex}" readonly/>
  <select name="partName[]" id="partName${partIndex}" class="form-control-sm partNameDropdown select2" onchange="setPartsData(${partIndex})" style="width: 100%;">
    ${$("#parts-dropdown").html()}
  </select>
</td>
<td><input type="number" step=".01" name="partPrice[]" id="partPrice${partIndex}" placeholder="Price" class="form-control-sm"></td>
<td><input type="number" step=".01" name="partDiscountPrice[]" id="partDiscountPrice${partIndex}" placeholder="Discounted Price" class="form-control-sm" readonly></td>
<td><input type="text" name="partNumber[]" id="part_number${partIndex}" placeholder="Enter Part Number" class="form-control-sm"></td>
<td><input type="text" name="partHsnCode[]" id="partHsnCode${partIndex}" placeholder="Enter HSN code" class="form-control-sm"></td>
<td><input type="number" step=".01" name="partQty[]" id="partQTY${partIndex}" placeholder="QTY" class="form-control-sm partGst" style="width: 6rem;" value="0"></td>
<td>
  <input type="number" step=".01" name="partCgst[]" id="partCgst${partIndex}" placeholder="CGST%" class="form-control-sm partGst" style="width: 6rem;" value="0">
  <input type="hidden" name="partCgstValue[]" id="partCgstValue${partIndex}" value="0">
</td>
<td>
  <input type="number" step=".01" name="partSgst[]" id="partSgst${partIndex}" placeholder="SGST%" class="form-control-sm partGst" style="width: 6rem;" value="0">
  <input type="hidden" name="partSgstValue[]" id="partSgstValue${partIndex}" value="0">
</td>
<td><input type="text" name="partTotal[]" id="partTotal${partIndex}" class="form-control-sm partTotal" readonly></td>
<td><button type="button" name="removePart" id="removePart${partIndex}" class="btn btn-danger btn-sm w-100 removePartButton">Remove</button></td>
</tr>`;
    $("#partTable tbody").append(html_code);
    $('#partName' + partIndex).select2();

    bindEvents();
});

// Populate fields when selecting a part
function setPartsData(index) {
    const selected = $('#partName' + index + ' option:selected');
    $('#partId' + index).val(selected.data('part-id') || '');
    $('#partPrice' + index).val(selected.data('sale-price') || 0);
    $('#partDiscountPrice' + index).val(selected.data('sale-price') || 0);
    $('#part_number' + index).val(selected.data('part-number') || '');
    $('#partHsnCode' + index).val(selected.data('hsn-code') || '');
    $('#partCgst' + index).val(selected.data('cgst') || 0);
    $('#partSgst' + index).val(selected.data('sgst') || 0);

    calculatePartFinalTotal();
}

// Bind events to all inputs that trigger recalculation
function bindEvents() {
    $('.partGst').off('input').on('input', calculatePartFinalTotal);
    $('.partNameDropdown').off('change').on('change', function () {
        const id = $(this).attr('id').replace('partName', '');
        setPartsData(id);
    });
    $('.removePartButton').off('click').on('click', function () {
...
function calculatePartFinalTotal() {
    let grandTotal = 0;
    let globalDiscount = parseFloat($('#partDiscount').val()) || 0;

    $('.partIndex').each(function () {
        let idx = $(this).val();

        let price = parseFloat($('#partPrice' + idx).val()) || 0;
        let qty = parseFloat($('#partQTY' + idx).val()) || 0;
        let cgst = parseFloat($('#partCgst' + idx).val()) || 0;
        let sgst = parseFloat($('#partSgst' + idx).val()) || 0;

        // Discounted price
        let discountedPrice = price - ((price * globalDiscount) / 100);
        $('#partDiscountPrice' + idx).val(discountedPrice.toFixed(2));
       // console.log(discountedPrice);
        // GST per item
        let perCgst = (discountedPrice * cgst) / 100;
        let perSgst = (discountedPrice * sgst) / 100;

        $('#partCgstValue' + idx).val((perCgst * qty).toFixed(2));
        $('#partSgstValue' + idx).val((perSgst * qty).toFixed(2));

        let totalPerItem = (discountedPrice + perCgst + perSgst) * qty;
        $('#partTotal' + idx).val(totalPerItem.toFixed(2));

        grandTotal += totalPerItem;
    });

    $('#partGrandTotal').val(grandTotal.toFixed(2));
}
...
html_code += '<td><input type="hidden" name="packageIndex[]" value="' + packageIndex + '" readonly/>';
html_code += '<input type="hidden" name="packageId[]" id="packageId' + packageIndex + '" class="form-control-sm" readonly/>';

html_code += '<select name="packageName[]" id="packageName' + packageIndex + '" class="form-control-sm packageDropdown" onchange="setPackageData(' + packageIndex + ')">';
html_code += '<option value="">Select Service Package</option>';
html_code += $("#packageName1").html(); 
html_code += '</select></td>';

html_code += '<td><input type="number" step=".01" name="packagePrice[]" id="packagePrice' + packageIndex + '" placeholder="Package Price" class="form-control-sm" readonly></td>';

html_code += '<td><input type="number" step=".01" name="discountprice[]" id="discountprice' + packageIndex + '" placeholder="Discount Price" class="form-control-sm" readonly></td>';

html_code += '<td><input type="text" step=".01" name="hsncode[]" id="hsncode' + packageIndex + '" placeholder="Hsn Code" class="form-control-sm" readonly></td>';

html_code += '<td><input type="number" step=".01" name="stock[]" id="stock' + packageIndex + '" placeholder="QTY" class="form-control-sm partGst" style="width: 6rem;" value="0"></td>';

html_code += '<td><input type="number" step=".01" name="cgst_percentage[]" id="cgst_percentage' + packageIndex + '" placeholder="CGST" class="form-control-sm" readonly></td>';

html_code += '<td><input type="number" step=".01" name="sgst_percentage[]" id="sgst_percentage' + packageIndex + '" placeholder="SGST" class="form-control-sm" readonly></td>';

html_code += '<td><input type="text" name="partTotal[]" id="partTotal' + packageIndex + '" class="form-control-sm partTotal" readonly></td>';

html_code += '<td><button type="button" name="removePart" id="removePart' + packageIndex + '" class="btn btn-danger btn-sm w-100 removePartButton">Remove</button></td>';

$("#packageTable").append(html_code);
$('#partName' + packageIndex).select2();
});



function setPackageData(index) {
    var selectedOption = $('#package' + index).find(':selected');

    var packageId = selectedOption.data('package-id');
    var salePrice = selectedOption.data('packageprice');
    var discountPrice = selectedOption.data('discountprice');
    var hsnCode = selectedOption.data('hsncode');
    var cgst = selectedOption.data('cgst_percentage');
    var sgst = selectedOption.data('sgst_percentage');

    $('#packageId' + index).val(packageId);
    $('#packagePrice' + index).val(salePrice);
    $('#discountprice' + index).val(discountPrice);
    $('#hsncode' + index).val(hsnCode);
    $('#cgst_percentage' + index).val(cgst);
    $('#sgst_percentage' + index).val(sgst);
...
function calculatePartFinalTotal() {
    ...
    $('tr[id^="packageRow"]').each(function () {
        var rowId = this.id.replace('packageRow', '');
        var price = parseFloat($('#discountprice' + rowId).val()) || 0;
        var qty = parseFloat($('#stock' + rowId).val()) || 0;
        var total = price * qty;

        $('#partTotal' + rowId).val(total.toFixed(2));
        finalTotal += total;
    });
...
$(document).on('keyup change', '.partGst', function () {
    calculatePartFinalTotal();
});
...
function loadInventoryOptions() {
  $.ajax({
    ...
    success: function(options) {
      ...
      // Update all dropdowns with new options
      $('.partNameDropdown').each(function () {
        $(this).html('<option value="">Select Parts From Inventory</option>' + options);
      });

      // Re-initialize Select2 after setting new options
      initSelect2();
    },
    error: function() {
...
loadInventoryOptions();
initSelect2(); // Initial setup in case options are already present
