...
/////////////////// display Cars ///////////////////////
if(isset($_SESSION['id'])){
    function display_cars($conn,$g_id)
    { 
        $g_id= $_SESSION['id'];
    
        $sql = "SELECT v.*, c.cus_mob 
        FROM all_vehicle v 
        JOIN customer c ON v.g_id = c.g_id 
        WHERE v.g_id = $g_id 
...
if (isset($_SESSION['id'])) {
    function display_cars1($conn, $g_id)
    {
        $g_id = $_SESSION['id'];

        // Joining all_vehicle with customer table on g_id to fetch cus_mob
        $sql = "SELECT v.*, c.cus_mob 
                FROM all_vehicle v 
                JOIN customer c ON v.g_id = c.g_id 
                WHERE v.g_id = $g_id 
...
$getGarageName = "SELECT g_name FROM call_login WHERE g_id = '$gid'";
$res = mysqli_query($conn, $getGarageName);
$data = mysqli_fetch_assoc($res);

$garage_name = strtoupper(preg_replace("/[^A-Z]/", "", $data['g_name']));
$prefix = substr($garage_name, 0, 3);
...
";
$result = mysqli_query($conn, $getLastJobCard);
$row = mysqli_fetch_assoc($result);

$next_no = $row['last_no'] ? $row['last_no'] + 1 : 1;
$job_card_no = $prefix . '-' . $next_no;
    ...
    // $sql="INSERT INTO `jobcard`(`g_id`,'c_id`,v_id`,`uid`,`invoice_no`,`odometer`,`fuelmeter`,`inventory`,`img1`,`img2`,`totalprice`) 
    // VALUES ('$gid','$cid','$vid','$uid','$invoice_no','$odometer','$fuelmeter','$inventry', '" . implode(",", $img1_uploaded_files) . "','" . implode(",", $img2_uploaded_files) . "','$totalprice')";
    
    $sql = "INSERT INTO `jobcard`(`g_id`,`c_id`,`v_id`,`uid`,`invoice_no`,`odometer`,`fuelmeter`,`insexpiry`,`inventory`,`insurance_company`,`img1`,`img2`,`totalprice`,`dueamount`,`status`, `part_discount`, `service_discount`,`job_card_type`,`job_card_no`) VALUES ('$gid','$cid','$vid','$uid','$invoice_no','$odometer','$fuelmeter','$insexpiry','$inventry','$insurance_company', '" . implode(",", $img1_uploaded_files) . "','" . implode(",", $img2_uploaded_files) . "','$finalTotal','$finalTotal','P','$part_discount','$service_discount','$job_card_type','$job_card_no')";
    $res=mysqli_query($conn,$sql);
    if($res==1){
        ...
        foreach ($partIndex as $key => $value) {
            ...
            $insert_code_items = "INSERT INTO `jobcode_service_items`(
                g_id, c_id, v_id, uid, service, price, discounted_price,partname,partPrice,partDiscountPrice, part_number,
                hsn_code, gst, cgst_percentage, cgst_value, sgst_percentage, sgst_value, qty, total, p_s,
                job_card_type, voice_of_customer, instruction_of_mechanic, job_card_no, service_due_date, package, packageprice,discountprice,hsncode,pqty,pcgst,psgst,totalcost,discountper,totalpay) 
            VALUES (
                '$gid','$cid','$vid','$uid','$partName[$key]','$partPrice[$key]','$partDiscountPrice[$key]','$partNumber[$key]',
                '$partHsnCode','$totalGst', '$partCgst[$key]','$partCgstValue[$key]','$partSgst[$key]',
                '$partSgstValue[$key]','$partQty[$key]','$partTotal[$key]', '1','$job_card_type','$voice_of_customer',
                '$instruction_of_mechanic','$job_card_no','$service_due_date','$package','$packageprice','$packageDiscountPrice','$packageHsnCode','$packageQty','$packageCgst','$packageSgst','$packageTotal','$package_discount','$packageGrandTotal')";   
        }   
    }
    for ($i = 0; $i < count($packageIds); $i++) {
        $package = mysqli_real_escape_string($conn, $packageIds[$i]);
        ...
        $packageDiscountPrice = mysqli_real_escape_string($conn, $packageDiscountPrice[$i]);
        $packageHsnCode = mysqli_real_escape_string($conn, $packageHsnCode[$i]);
        $packageQty = mysqli_real_escape_string($conn, $packageQty[$i]);
        $packageCgst = mysqli_real_escape_string($conn, $packageCgst[$i]);
        $packageSgst = mysqli_real_escape_string($conn, $packageSgst[$i]);
        $packageTotal = mysqli_real_escape_string($conn, $packageTotal[$i]);
        $package_discount = mysqli_real_escape_string($conn, $package_discount[$i]);
        $packageGrandTotal = mysqli_real_escape_string($conn, $packageGrandTotal[$i]);

        foreach ($srno as $key => $value) 
        {
            if(!empty($serviceName[$key]))
            {
                $totalGst = $serviceCgst[$key] + $serviceSgst[$key];

                $insert_code_items="INSERT INTO `jobcode_service_items`(g_id, c_id, v_id, uid, service, price, discounted_price,partname,partPrice,partDiscountPrice,part_number,
                 hsn_code, gst, cgst_percentage, cgst_value, sgst_percentage, sgst_value, total, p_s,job_card_type,voice_of_customer,
                 instruction_of_mechanic,job_card_no,service_due_date,package,packageprice,discountprice,hsncode,pqty,pcgst,psgst,totalcost,discountper,totalpay) 
                VALUES ('$gid','$cid','$vid','$uid','$serviceName[$key]','$ser[$key]','$serDiscountedPrice[$key]','$partName[$key]','$partPrice[$key]','$partDiscountPrice[$key]','$partNumber[$key]',
                '$hsn[$key]','$totalGst','$serviceCgst[$key]','$serviceCgstValue[$key]','$serviceSgst[$key]','$serviceSgstValue[$key]','$totalprice[$key]', '2',
                '$job_card_type','$voice_of_customer','$instruction_of_mechanic','$job_card_no','$service_due_date','$package','$packageprice','$packageDiscountPrice','$packageHsnCode','$packageQty','$packageCgst','$packageSgst','$packageTotal','$package_discount','$packageGrandTotal')";
        ...
        // Update inventory for both parts and services
...
function update_jobcard1($conn) {
    try {
        ...
        foreach ($partIndex as $key => $value) {
            ...
            {
               $insert_code_items="INSERT INTO `jobcode_service_items`(p_s, g_id, c_id, v_id, uid, service, price, discounted_price, part_number, hsn_code, gst, cgst_percentage, cgst_value, sgst_percentage, sgst_value, qty, total,
            ...
            {
                $update_code_items = "UPDATE `jobcode_service_items` SET `g_id`='$g_id',`c_id`='$c_id',`v_id`='$v_id',`uid`='$uid',`service`='$partName[$key]',`price`='$partPrice[$key]',`discounted_price`='$partDiscountPrice[$key]',`part_number`='$partNumber[$key]',`hsn_code`='$partHsnCode[$key]',`gst`='$totalGst',`cgst_percentage`='$partCgst[$key]',`cgst_value`='$partCgstValue[$key]',`sgst_percentage`='$partSgst[$key]',`sgst_value`='$partSgstValue[$key]',`qty`='$partQty[$key]',`total`='$partTotal[$key]',`voice_of_customer`='$voice_of_customer',`instruction_of_mechanic`='$instruction_of_mechanic',`service_due_date`='$service_due_date' WHERE `id`=".$part_service_id[$key];
...
function totaldueamount($conn)
{
    ...
    $sql = "SELECT SUM(dueamount) AS total_due FROM jobcard WHERE g_id = '$g_id' AND work_status = 0";
    $res = mysqli_query($conn, $sql);
...
function display_jobcard($conn)
{
    ...
    $sql_1 = "SELECT jobcard.*, jobcard.job_card_type,jobcard.status,jobcard.totalPrice, jobcard.dueamount,customer.cus_name, customer.cus_mob, all_vehicle.registration, 
    all_vehicle.carbrand, all_vehicle.carmodel 
    FROM jobcard 
    JOIN customer ON jobcard.c_id = customer.c_id 
    JOIN all_vehicle ON jobcard.v_id = all_vehicle.v_id 
    WHERE $where 
...
$jobCardLink = "https://merigarage.com/GarageAdmin/jobcard_approvel.php?id=" . $row['id'];
...
?>
            ...
            <td class="text-center" style="white-space: nowrap;">
                ...
                <a href="https://api.whatsapp.com/send/?phone=%2B91<?php echo $row['cus_mob'];?>&text=Job Card Approval:
                                    ...
                                    I hope this message finds you well. This is <?php if (isset($_SESSION['user'])) { echo $_SESSION['user']; }; ?>. We would like to inform you that your <?php echo $row['carmodel'];?> is due for routine maintenance service.                                                               Please approve your Job Card through this link: [Job Card Approval Link]: https://merigarage.com/GarageAdmin/jobcard_approvel.php?id=<?php echo $row['id'];?>

                Our team of certified technicians is ready to provide the necessary services after your approval. We take pride in delivering high-quality service to our valued customers, and your satisfaction is our top priority..!                                                        Best regards,                                       <?php if (isset($_SESSION['user'])) { echo $_SESSION['user']; }; ?>                                                     Contact: <?php if (isset($_SESSION['user'])) { echo $_SESSION['g_mob']; }; ?> 
                Address: <?php if (isset($_SESSION['user'])) { echo $_SESSION['g_address']; }; ?>
    ...
    $(document).ready(function() {
        $('.dropdown-select1').change(function() {
            ...
            $.ajax({
                ...
                data: { jobId: jobId, mechanicId: mechanicId },
...
if (isset($_POST['add_product'])) {
   ...
   $gid = $_SESSION['id'];
    ...
    $sql="INSERT INTO `inventory`(`g_id`,`pid`,`Product`,`Photo`,`PartNumber`,`HsnCode`,`Category`,`Location`,`Stock`,`CostPrice`,`SalePrice`,`cgst_percentage`,`sgst_percentage`) 
    VALUES ('$gid','$pid','$Product','$file','$PartNumber','$partHsnCode','$Category','$Location','$Stock','$CostPrice','$SalePrice','$cgst_percentage','$sgst_percentage')";
...
if (isset($_POST['add_package'])) {
   $gid = $_SESSION['id'];
    ...
    $sql="INSERT INTO `servicepackage`(`g_id`,`pid`,`package`,`packageprice`,`discountprice`,`hsncode`,`stock`,`cgst_percentage`,`sgst_percentage`) 
    VALUES ('$gid','$pid','$package','$packageprice','$discountprice','$hsncode','$stock','$cgst_percentage','$sgst_percentage')";
...
function update_product($conn){
    ...
    if(mysqli_num_rows($check_result) > 0){
        ...
        $update_sql = "UPDATE inventory SET g_id = '$gid', Product = '$Product', Photo = '$file', PartNumber = '$PartNumber', HsnCode = '$partHsnCode', Category = '$Category', Location = '$Location', Stock = '$Stock', CostPrice = '$CostPrice', SalePrice = '$SalePrice', cgst_percentage = '$cgst_percentage', sgst_percentage = '$sgst_percentage' WHERE id = '$id'";
        $update_result = mysqli_query($conn, $update_sql);
...
function update_package($conn){
    ...
    if(mysqli_num_rows($check_result) > 0){
        ...
        $update_sql = "UPDATE servicepackage SET g_id = '$gid', package = '$package', packageprice = '$packageprice' WHERE id = '$id'";
        $update_result = mysqli_query($conn, $update_sql);
