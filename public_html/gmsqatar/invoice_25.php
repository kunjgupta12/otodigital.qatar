                                        ...
                                        while ($row = mysqli_fetch_assoc($res)) 
                                        {
                                           ...
                                           // $totalTax += ($row['partcgst'] + $row['partcgst']) * $row['partqty'];
                                            ...
                                            if($row['partsgst'] > 0)
                                            {
                                                ...
                                                {
                                                    $gst_summary[$gstKey]["totalAmount"] += $base_price * $row['partqty'];
                                                   echo $gst_summary[$gstKey]["tax"] += ($base_price * $row['partsgst']/100) * $row['partqty'];
                                                }
                                                else
                                                {
                                                    $pgst = array(
                                                        ...
                                                        "totalAmount" => $base_price * $row['partqty'],
                                                        "tax" => ($base_price * $row['partsgst']/100) * $row['partqty']
                                                    );
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $query = "SELECT j.*, s.package AS service_package_name, s.packageprice AS service_package_price
                                     FROM jobcode_service_items j
                                     LEFT JOIN servicepackage s ON j.package = s.id
                                     WHERE j.uid = '$itemId' AND j.p_s = '3' AND j.g_id = '".$_SESSION['id']."'";
                                    $res = mysqli_query($conn, $query);
                                    if (mysqli_num_rows($res) > 0) 
                                    {
                                        $i = 1;
                                        ...
                                        {   
                                          ...
                                          $totalDiscount += (($row['service_package_price'] - $row['discountprice']) * $row['pqty']);
                                            ...
                                            if($row['psgst'] > 0)
                                            {
                                                ...
                                                {
                                                    $gst_summary[$gstKey]["totalAmount"] += $base_price * $row['pqty'];
                                                    $gst_summary[$gstKey]["tax"] += ($base_price*$row['psgst']/100)* $row['pqty'];
                                                }
                                                else
                                                {
                                                    $pgst = array(
                                                        "title" => "SGST @".$row['pcgst']."% (Package)",
                                                        "totalAmount" => $base_price * $row['pqty'],
                                                        "tax" => ($base_price*$row['psgst']/100)* $row['pqty']
                                                    );
                                ...
                                <tr>
                                    ...
                                    <td> ₹ <?= number_format($row['discountprice'],2) ?> (<?= $packageDiscount; ?>%)</td>  
                                    <td>₹ <?= number_format(($base_price ), 2); ?></td>

                                    <td><?= $row['pqty']; ?></td>  
                                    <td>PCS</td>
                                    <td>₹ <?= number_format(($base_price)*$row['pqty'], 2); ?></td>
                                    ...
                                    if (mysqli_num_rows($res) > 0) 
                                    {
                                        ...
                                        while ($row = mysqli_fetch_assoc($res)) 
                                        {
                                            ...
                                            if($row['cgst_percentage'] > 0)
                                            {
                                                ...
                                                {
                                                    $pgst = array(
                                                        "title" => "CGST @".$row['cgst_percentage']."% (Service)",
                                            ...
                                            if($row['sgst_percentage'] > 0)
                                            {
                                                ...
                                                {
                                                    $pgst = array(
                                                        "title" => "SGST @".$row['sgst_percentage']."% (Service)",
                                                        "totalAmount" => $row['discounted_price'] * $row['qty'],
